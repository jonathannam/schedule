...
import { createMediaPrivilegeAPI, updateMediaPrivilegeAPI } from '../../../service/schedule/schedule.privilege.service';
...
export default function PrivilegeNewForm({ isEdit, isView, item }) {
  ...
  const onSubmit = async (data) => {
    ...
    if (resp.code === '200') {
      ...
      navigate(PATH_DASHBOARD.schedule.privileges);
    } else
          ...
          <Stack spacing={3}>
            ...
            </Card>
            {
              isView ? (<Button
                ...
                to={`${PATH_DASHBOARD.schedule.root}/privilege/${item?.id}/edit`}
                size="large"
