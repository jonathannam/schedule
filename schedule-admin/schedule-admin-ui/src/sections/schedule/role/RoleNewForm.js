...
import { createMediaRoleAPI, updateMediaRoleAPI } from '../../../service/schedule/schedule.role.service';
...
export default function RoleNewForm({ isEdit, isView, currentRole }) {
  ...
  const onSubmit = async (data) => {
    ...
    if (resp.code === '200') {
      ...
      navigate(PATH_DASHBOARD.schedule.roles);
    } else
          ...
          <Stack spacing={3}>
            ...
            </Card>
            {
              isView ? (<Button
                ...
                to={`${PATH_DASHBOARD.schedule.root}/role/${currentRole?.id}/edit`}
                size="large"
